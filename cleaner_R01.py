...
def add_calculated_columns(df):
    ...
    for col_name, tags in COLUMN_MAPPING.items():
        ...
        if tags["PTB"] in df.columns and tags["PTT"] in df.columns:
            df[dp_col] = df[tags["PTB"]] - df[tags["PTT"]]
        else:
            df[dp_col] = np.nan
        if tags["Reflux_Flow"] in df.columns and tags["Top_Product_Flow"] in df.columns:
            df[reflux_ratio_col] = df[tags["Reflux_Flow"]] / df[tags["Top_Product_Flow"]]
        else:
            df[reflux_ratio_col] = np.nan
    ...
    for exchanger_name, tags in HEAT_EXCHANGERS.items():
        ...
        if all(tag in df.columns for tag in [flow_tag, supply_temp_tag, return_temp_tag]):
            if tags["fluid"] == "water":
                df[heat_duty_col] = (
                    df[flow_tag] * RHO_WATER * CP_WATER * (df[return_temp_tag] - df[supply_temp_tag])
                )
            elif tags["fluid"] == "tf66":
                df[heat_duty_col] = (
                    df[flow_tag] * RHO_TF66 * CP_TF66 * (df[return_temp_tag] - df[supply_temp_tag])
                )
        else:
            df[heat_duty_col] = np.nan
    return df
...
def process_scada_data_in_range(start_timestamp, end_timestamp):
    ...
    try:
        ...
        # Get the list of calculated columns to be added to the schema
        calculated_cols = []
        for col in COLUMN_MAPPING.keys():
            calculated_cols.append(f"DP_{col}")
            calculated_cols.append(f"Reflux_Ratio_{col}")
        for exchanger in HEAT_EXCHANGERS.keys():
            calculated_cols.append(f"HeatDuty_{exchanger}")
        
        # Update schema to include all tags and calculated columns
        ...
        # --- Perform Calculations immediately after resampling ---
            ...
            if tag in processed_df.columns:
                ...
                if faulty_mask.any():
                    ...
                    for ts in processed_df[faulty_mask].index:
                        detailed_log.append({'DateAndTime': ts, 'Instrument': tag, 'Anomaly_Type': 'Faulty Value', 'Value': processed_df.loc[ts, tag], 'Notes': f'Detected faulty value {FAULTY_VALUE}.'})
                ...
                if tag in ti_tags:
                    ...
                    if spike_mask.any():
                        ...
                        for ts in processed_df[spike_mask].index:
                            detailed_log.append({'DateAndTime': ts, 'Instrument': tag, 'Anomaly_Type': 'Temp Spike', 'Value': processed_df.loc[ts, tag], 'Notes': 'Sudden temperature spike detected.'})
                ...
                if tag in OPERATING_RANGES:
                    ...
                    if excursion_mask.any():
                        ...
                        for ts in processed_df[excursion_mask].index:
                            detailed_log.append({'DateAndTime': ts, 'Instrument': tag, 'Anomaly_Type': 'Process Excursion', 'Value': processed_df.loc[ts, tag], 'Notes': 'Value outside of operating range.'})

        # --- Handle Negative Flow Values ---
        print("--- Applying Rule for Negative Flow Values ---")
        fi_tags = TAG_TYPES["FI"]
        for tag in fi_tags:
            if tag in processed_df.columns:
                negative_flow_mask = processed_df[tag] < 0
                if negative_flow_mask.any():
                    processed_df.loc[negative_flow_mask, 'is_faulty_sensor'] = True
                    processed_df.loc[negative_flow_mask, 'imputed_with'] = 'Negative Flow Replaced'
                    processed_df.loc[negative_flow_mask, 'notes'] += 'Negative flow detected and replaced. '
                    anomaly_counts['is_faulty_sensor'] += negative_flow_mask.sum()
                    for ts in processed_df[negative_flow_mask].index:
                        detailed_log.append({'DateAndTime': ts, 'Instrument': tag, 'Anomaly_Type': 'Negative Flow', 'Value': processed_df.loc[ts, tag], 'Notes': 'Negative flow detected.'})
                ...
                if current_tag in processed_df.columns and next_tag in processed_df.columns:
                    ...
                    if anomaly_mask.any():
                        ...
                        for ts in processed_df[anomaly_mask].index:
                            detailed_log.append({'DateAndTime': ts, 'Instrument': current_tag, 'Anomaly_Type': 'Temp Profile Anomaly', 'Value': processed_df.loc[ts, current_tag], 'Notes': f'Temperature {current_tag} is less than {next_tag}.'})
            ...
            if "PTT" in pressure_tags and "PTB" in pressure_tags:
                ...
                if ptt in processed_df.columns and ptb in processed_df.columns:
                    ...
                    if anomaly_mask.any():
                        ...
                        for ts in processed_df[anomaly_mask].index:
                            detailed_log.append({'DateAndTime': ts, 'Instrument': ptt, 'Anomaly_Type': 'Pressure Profile Anomaly', 'Value': processed_df.loc[ts, ptt], 'Notes': f'Pressure at top ({ptt}) is greater than pressure at bottom ({ptb}).'})
        ...
        for li_tag in li_tags:
            ...
            if li_tag in processed_df.columns and fi_tag in processed_df.columns:
                ...
                if anomaly_mask.any():
                    ...
                    for ts in processed_df[anomaly_mask].index:
                        detailed_log.append({'DateAndTime': ts, 'Instrument': li_tag, 'Anomaly_Type': 'Flow/Level Anomaly', 'Value': processed_df.loc[ts, li_tag], 'Notes': f'Tank level increasing while flow is low.'})
        ...
        if all(tag in processed_df.columns for tag in ft_shutdown_tags):
            ...
            if all_ft_zero.any():
                ...
                for ts in processed_df[all_ft_zero].index:
                    detailed_log.append({'DateAndTime': ts, 'Instrument': 'Multiple', 'Anomaly_Type': 'Plant Shutdown', 'Value': None, 'Notes': 'All major flow meters are at or near zero.'})
        ...
        if 'TI215' in processed_df.columns:
            ...
            if boiler_mask.any():
                ...
                for ts in processed_df[boiler_mask].index:
                    detailed_log.append({'DateAndTime': ts, 'Instrument': 'TI215', 'Anomaly_Type': 'Boiler Anomaly', 'Value': processed_df.loc[ts, 'TI215'], 'Notes': 'Boiler temperature is outside its normal operating range.'})


        # --- Rule 5: Handling and Imputing Faulty Values ---
        print("--- Applying Rule 5: Handling and Imputing ---")
        
        # Replace the FAULTY_VALUE with NaN so it can be interpolated
        for tag in all_tags_from_csv:
            if tag in processed_df.columns:
                processed_df[tag] = processed_df[tag].replace(FAULTY_VALUE, np.nan)
        ...
        for tag in all_tags_from_csv:
            if tag in processed_df.columns:
                ...
                processed_df[tag].fillna(method='ffill', inplace=True)
        ...
        final_df_columns = ['DateAndTime'] + all_tags_from_csv + calculated_cols + flag_cols + ['imputed_with', 'notes']
        ...
        for tag in all_tags_from_csv:
            if tag in processed_df.columns:
                ...
                # Count definitive faulty conditions
                faulty_mask = (processed_df[tag] == FAULTY_VALUE)
                if tag in TAG_TYPES["TI"]:
                    diff = processed_df[tag].diff().abs()
                    spike_mask = (diff > TI_SPIKE_THRESHOLD) & (processed_df[tag].shift(-1).abs() < processed_df[tag].abs())
                    faulty_mask = faulty_mask | spike_mask
                if tag in TAG_TYPES["FI"]:
                    negative_flow_mask = processed_df[tag] < 0
                    faulty_mask = faulty_mask | negative_flow_mask
