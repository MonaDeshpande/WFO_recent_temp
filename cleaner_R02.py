...
# --- USER INPUT ---
START_DATE = "2025-09-08 11:15:00"
END_DATE = "2025-09-17 11:00:00"
...
TAG_TYPES = {
    ...
    "FI": ["FI08", "FI09", "FI10", "FT08", "FT09", "FT10"]
}
...
def add_calculated_columns(df):
    ...
    for exchanger_name, tags in HEAT_EXCHANGERS.items():
        ...
        if all(tag in df.columns for tag in required_heat_tags):
            if tags["fluid"] == "water":
                df[heat_duty_col] = (df[flow_tag] * RHO_WATER * CP_WATER * (df[return_temp_tag] - df[supply_temp_tag]))
            elif tags["fluid"] == "tf66":
                df[heat_duty_col] = (df[flow_tag] * RHO_TF66 * CP_TF66 * (df[return_temp_tag] - df[supply_temp_tag]))
...
def process_scada_data_in_range(start_timestamp, end_timestamp):
    ...
    try:
        ...
        # NOTE: This has been moved here to ensure calculations run on complete data
        processed_df, calculated_cols = add_calculated_columns(processed_df)
        
        # --- Basic Data Smoothing (AFTER Imputation) ---
        # NOTE: This has been moved here to ensure smoothing runs on complete data
